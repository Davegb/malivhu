...
def submit(request):
    ...
    if request.method =='POST':
        ...
        p = executeCommand("\"mkdir VirusProteins/webserver/files/" + jobid + "\"")
        if p != "":
            return render(request, 'vhppi/submit.html', {"jobid": "There was an error trying to submit your job. Please, try again later or contact us.", "virus": "", "human": ""})
        p = executeCommand("\"echo '" + seqV + "' > VirusProteins/webserver/files/" + jobid + "/virus.fasta\"")
        if p != "":
            return render(request, 'vhppi/submit.html', {"jobid": "There was an error trying to submit your job. Please, try again later or contact us.", "virus": "", "human": ""})
        if p4 or predictVirus != "None":
            p = executeCommand("\"echo '" + seqH + "' > VirusProteins/webserver/files/" + jobid + "/human.fasta\"")
            if p != "":
                return render(request, 'vhppi/submit.html', {"jobid": "There was an error trying to submit your job. Please, try again later or contact us.", "virus": "", "human": ""})
...
def submitPhase4(request, jobId):
    if request.method =='POST':
        ...
        p = executeCommand("\"echo '" + seqH + "' > VirusProteins/webserver/files/" + str(jobId) + "/human.fasta\"")
        if p != "":
...
def results(request, jobId):
    ...
    if "runP2" in request.POST:
        p = executeCommand("\"echo \"DONE\" > ./VirusProteins/webserver/files/" + str(jobId) + "/predictP2.out \"")
        if p != "":
        ...
        p = executeCommand("\"echo \"DONE\" > ./VirusProteins/webserver/files/" + str(jobId) + "/predictP3.out \"")
        if p != "":
        ...
        p = executeCommand("\"echo \"DONE\" > ./VirusProteins/webserver/files/" + str(jobId) + "/predictP4.out \"")
        if p != "":
    ...
    p = executeCommand("\"cat ./VirusProteins/webserver/files/" + str(jobId) + "/virusPsipred.out ; cat ./VirusProteins/webserver/files/" + str(jobId) + "/humanPsipred.out\"")
    structures = p.split("\n")

    # Get phases status
    p = executeCommand("\"cat ./VirusProteins/webserver/files/" + str(jobId) + "/predictP1.out ; echo '' ; cat ./VirusProteins/webserver/files/" + str(jobId) + "/predictP2.out ; echo '' ; cat ./VirusProteins/webserver/files/" + str(jobId) + "/predictP3.out ; echo '' ; cat ./VirusProteins/webserver/files/" + str(jobId) + "/predictP4.out \"")
    phases = p.split("\n")
    ...
    if phases[0].strip() == "YES":
        ...
        p = executeCommand("cat ./VirusProteins/webserver/files/" + str(jobId) + "/phase1.out")
        phase1 = []
        lines = p.splitlines()
        for line in lines:
    ...
    if phases[1].strip() == "YES":
        ...
        p = executeCommand("cat ./VirusProteins/webserver/files/" + str(jobId) + "/phase2.out")
        phase2 = []
        lines = p.splitlines()
        for line in lines:
            proteinData = line.split()
    ...
    if phases[2].strip() == "YES":
        ...
        p = executeCommand("cat ./VirusProteins/webserver/files/" + str(jobId) + "/phase3.out")
        phase3 = []
        lines = p.splitlines()
        for line in lines:
            proteinData = line.split()
    ...
    if phases[3].strip() == "YES":
        ...
        p = executeCommand("cat ./VirusProteins/webserver/files/" + str(jobId) + "/phase4.out")
        phase4 = []
        lines = p.splitlines()
        for line in lines:
            proteinData = line.split()
...
def secondaryStructure(request, jobId, protein):
    ...
    p = executeCommand("cat ./VirusProteins/webserver/files/" + str(jobId) + "/" + protein + ".horiz")
    if(p.strip() == "YES"):
...
def checkJob(request):
    if request.method == 'POST':
        ...
        p = executeCommand("\"cat ./VirusProteins/webserver/files/" + str(jobId) + "/predictP1.out\"")
        if p == "DONE":
...
def executeCommand(command):
    ssh = "ssh dguevara@biocluster.usu.edu "
    if platform == "Linux":
        p = subprocess.getoutput(ssh + command)
        return p
    elif platform == "Windows":
        p = subprocess.run(["C:/Program Files/Git/bin/bash.exe", "-c", ssh + command], capture_output=True)
